...
const double ref_v = 70.0*0.447;//unit: m/s

//tune the weights manually
const double weight_cte = 70.0;
const double weight_epsi = 1000.0;
const double weight_v = 20.0;
const double weight_delta = 50000.0;
const double weight_a = 1.0;
const double weight_delta_jerk = 50000.0;
const double weight_a_jerk = 1.0;
...
class FG_eval {
    public:
        ...
        void operator()(ADvector& fg, const ADvector& vars) {
            ...
            for (t = 0; t < N - 2; t++) {
              fg[0] += weight_delta_jerk*CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
              fg[0] += weight_a_jerk*CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
            }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //set the lower and upper limits of the delta to -25 and 25 degrees (values in radians);
  for (i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436332;
    vars_upperbound[i] = 0.436332;
  }

  //set the lower and upper limits of the acceleration (m/s^2);
...
#ifdef DEBUG_OUTPUT
  std::cout << "---------------output mpc solution----------------" << std::endl;
  std::cout << "--------delta: ";
  for (i = 0; i < N-1; i++){
